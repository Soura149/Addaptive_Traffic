...
OUTPUT_CSV_FILE = os.path.join(DATA_DIR, f"time_based_data_{SIMULATION_DURATION}s.csv")
...
def run_time_based_simulation():
    ...
    if GENERATE_CONTINUOUS_FLOW:
        ...
        if RANDOMIZE_PROBABILITIES:
            we_prob = round(random.uniform(0.1, 0.6), 2)
            ns_prob = round(random.uniform(0.1, 0.6), 2)
            print(f"Randomized Traffic Flow -> WE: {int(we_prob*100)}%, NS: {int(ns_prob*100)}%")
        else:
            print(f"Fixed Traffic Flow -> WE: {int(we_prob*100)}%, NS: {int(ns_prob*100)}%")
    ...
    cmd = [checkBinary('sumo-gui'), "-n", net_path, "-r", route_path, "--no-step-log", "true", "--no-warnings", "true", "--time-to-teleport", "-1"]
    traci.start(cmd)
