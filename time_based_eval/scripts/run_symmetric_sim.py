...
OUTPUT_CSV_FILE = os.path.join(DATA_DIR, f"symmetric_data_{SIMULATION_DURATION}s.csv")
...
def run_time_based_simulation():
    ...
    if GENERATE_CONTINUOUS_FLOW:
        ...
        if RANDOMIZE_PROBABILITIES:
            shared_prob = round(random.uniform(0.1, 0.6), 2)
            we_prob = shared_prob
            ns_prob = shared_prob
            print(f"Randomized Symmetric Traffic Flow -> WE: {int(we_prob*100)}%, NS: {int(ns_prob*100)}%")
        else:
            print(f"Fixed Symmetric Traffic Flow -> WE: {int(we_prob*100)}%, NS: {int(ns_prob*100)}%")
